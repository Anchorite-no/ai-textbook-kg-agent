...
REQUIRED_ACTIONS = {"merge", "keep", "remove", "refine", "conflict"}
...
def main() -> None:
    ...
    try:
        with TemporaryDirectory() as tmp:
            ...
            report = _score_integration(integration, elapsed_ms)
            assert report["required_actions_covered"] is True, report
            assert report["compression_target_met"] is True, report
            assert report["decision_evidence_completeness"] == 1.0, report
            assert report["node_count_changed"] is True, report

            output_path = settings.integration_data_dir / "stage8_integration_benchmark_latest.json"
...
def _prepare_graphs(tmp: str) -> list[str]:
    samples = {
        ...
        "book_c.md": (
            "# 第一章 leukocyte\n"
            "leukocyte 指白细胞，是免疫系统中执行防御功能的细胞，可迁移到炎症区域。\n"
            "# 第二章 动作电位\n"
            "动作电位是神经细胞膜电位在阈值刺激后快速上升再恢复的过程，"
            "该过程可以解释信号沿轴突传播以及离子通道时序变化。\n"
            "# 第三章 炎症\n"
            "炎症是组织应对损伤和病原刺激的防御性反应，但炎症严重时也会造成病理损伤。\n"
            "# 第四章 抗体\n"
            "抗体可以特异性结合抗原，属于免疫应答的重要分子。\n"
            "# 第五章 页码颜色\n"
            "页码颜色只是排版标记。页码颜色不是课程核心概念，整合时可以从正文压缩。\n"
        ),
    }
    client = TestClient(app)
...
def _score_integration(integration: dict[str, Any], elapsed_ms: float) -> dict[str, Any]:
    ...
    if decisions:
        evidence_complete = sum(
            ...
            if decision["reason"] and decision["confidence"] > 0 and decision["evidence_chunk_ids"] and decision["source_locators"]
        ) / len(decisions)
    return {
        "benchmark": "00_stage8_integration",
        "average_build_ms": round(elapsed_ms, 2),
        "required_actions_covered": REQUIRED_ACTIONS.issubset(actions),
        "action_coverage": {action: action in actions for action in sorted(REQUIRED_ACTIONS)},
        "decision_count": len(decisions),
        "integrated_concept_count": len(integration["integrated_concepts"]),
        "original_node_count": stats["original_node_count"],
        "integrated_node_count": stats["integrated_node_count"],
        "node_count_changed": stats["integrated_node_count"] < stats["original_node_count"],
        "node_reduction_ratio": stats["node_reduction_ratio"],
        "original_char_count": stats["original_char_count"],
        "retained_char_count": stats["retained_char_count"],
        "target_compression_ratio": stats["target_compression_ratio"],
        "compression_ratio": stats["compression_ratio"],
        "compression_target_met": stats["compression_ratio"] <= stats["target_compression_ratio"],
        "decision_evidence_completeness": round(evidence_complete, 4),
        "removed_node_count": stats["removed_node_count"],
        "merged_node_count": stats["merged_node_count"],
        "refined_node_count": stats["refined_node_count"],
